...
void ThorLevel::setDuration(int level, unsigned int dur){
  _levelDuration[_getIndexFromLevel(level)] = dur;
}

void ThorLevel::setNumColors(int level, int numColors){
  _levelNumColors[_getIndexFromLevel(level)] = numColors;
}
...
int ThorLevel::_checkLevel() {
  ...
  if(_level != previousLevel){
    _numColors = _levelNumColors[_getIndexFromLevel(_level)];
    _duration = _levelDuration[_getIndexFromLevel(_level)];
    _increment = _levelScoreIncrement[_getIndexFromLevel(_level)];
    _penalty = _levelScorePenalty[_getIndexFromLevel(_level)];
  }

  return _level;
}

unsigned int ThorLevel::getDuration() {
  return _duration;
}

int ThorLevel::getNumColors() {
  return _numColors;
}
